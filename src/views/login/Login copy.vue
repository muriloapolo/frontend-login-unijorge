...
export default {
  ...
  methods: {
    async handleLogin() {
      ...
      try {
        const response = await axios.post('http://localhost:3000/api/secretarios/login', {
          email: this.email,
        ...
        if (response.status === 200) {
          ...
          // Atualiza o estado reativo do router para que ele saiba que o usuário está autenticado
        ...
        this.mensagemErro = error.response?.data?.message || 'Erro ao conectar. Verifique sua conexão ou tente novamente.';
      } finally {
